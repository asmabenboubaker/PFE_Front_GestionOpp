...
export class IntsanceViewerComponent implements OnInit {




  typeIndex: any = 0;
  modelDiv: any = jQuery('#bpmnModel');
  ...
  ngOnInit() {
    ...
    this.http.patch(`${this.env.apiUrlkernel}` + 'workflow/instance/'+this.wfProcessID+'/json',{} ,{headers: this.headers}).subscribe((model:any[])=> {
      ...
      // data.childShapes[0].bounds.lowerRight.x = data.childShapes[0].bounds.lowerRight.x - data.childShapes[0].bounds.lowerRight.x / 4;
      // data.childShapes[0].bounds.lowerRight.y = data.childShapes[0].bounds.lowerRight.y - data.childShapes[0].bounds.lowerRight.y / 4;
      // data.childShapes[0].bounds.upperLeft.x = data.childShapes[0].bounds.upperLeft.x - data.childShapes[0].bounds.upperLeft.x / 4;
      // data.childShapes[0].bounds.upperLeft.y = data.childShapes[0].bounds.upperLeft.y - data.childShapes[0].bounds.upperLeft.y / 4;
      ...
      this.paper = Raphael(document.getElementById('bpmnModel'), 'auto', 'auto');
      ...
      if (elements) {
        for (let i = 0; i < elements.length; i++) {
          ...
          while (flow.waypoints[index]) {
            ...
            // flow.waypoints[index].x = flow.waypoints[index].x - flow.waypoints[index].x / 4;
            // flow.waypoints[index].y = flow.waypoints[index].y - flow.waypoints[index].y / 4;
    ...
    elements.forEach(elem => {
      if (elem.stencil.id === 'Pool') {
                  ...
                  if (elem['properties' + index].usertaskassignment.assignment.startDate[0] && elem['properties' + index].usertaskassignment.assignment.startDate[0].value !== null && elem['properties' + index].usertaskassignment.assignment.startDate[0].value !== 'null') {
                    elem['properties' + index].usertaskassignment.assignment.Assignee.forEach(assignment => {
                      if (elemTemp['properties' + index]) {
                        ...
                        elemTemp['properties' + index] = [{
                          name: 'Date Début',
                          type: 'list',
                          value: []
                        }, {name: 'Responsable', type: 'list', value: []}, {
                          name: 'Réalisé par',
                          type: 'list',
                          value: []
                        }, {name: 'Réalisé le', type: 'list', value: []}];
                        elemTemp['properties' + index][1].value.push(assignment.value);
                      }
                    });
                  }
              ...
              if (elem['properties'].usertaskassignment.assignment.assignee) {
                if (elem['properties'].usertaskassignment.assignment.assignee !== null) {
                  if (elemTemp['properties']) {
                    ...
                    elemTemp['properties'] = [{name: 'Date Début', type: 'list', value: []}, {
                      name: 'Responsable',
                      type: 'list',
                      value: []
                    }, {name: 'Réalisé par', type: 'list', value: []}, {name: 'Réalisé le', type: 'list', value: []}];
        ...
        const elemTemp = {
          ...
          x: parent.bounds.upperLeft.x + elem.bounds.upperLeft.x,
          y: parent.bounds.upperLeft.y + elem.bounds.upperLeft.y,
          width: (elem.bounds.lowerRight.x - elem.bounds.upperLeft.x),
          height: (elem.bounds.lowerRight.y - elem.bounds.upperLeft.y),
          type: 'ServiceTask',
          properties: []
        }
        if (elem.properties.active) {
