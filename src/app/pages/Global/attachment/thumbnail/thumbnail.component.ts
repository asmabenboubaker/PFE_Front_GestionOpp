...
export class ThumbnailComponent implements OnInit {
    ...
    @Input() cansigDigigo: Boolean = true;
    ...
    actionSignDigigo: ActionItem;
    actionSignCertif: ActionItem;
    actionSignUSB: ActionItem;
    ...
    signatureUUID;
    email = this.cookieService.get('mail');
    FileNameToSigned;
    signatureType;
    ConfirmerSigned = false;
    ...
    constructor(private sanitizer: DomSanitizer, private communService: CommunFuncService, private deviceService: DeviceDetectorService, private tokenStorage: TokenStorageService, private http: HttpClient, private fileservice: AttachementModuleService, private cookieService: CookieService, private env: EnvService, private translateService: TranslateService, private toastr: ToastrService,) {
        ...
        this.translateService.get(['ATTACHEMENT.viewe',
            ...
            'ATTACHEMENT.signatureDigogo',
            'ATTACHEMENT.signatureUSB',
            'ATTACHEMENT.signatureCertif',
            'ATTACHEMENT.sign',
            'ATTACHEMENT.MAJ',
            'ATTACHEMENT.open',
            ...
            this.actionSignDigigo = new ActionItem(16, res['ATTACHEMENT.signatureDigogo'], 'fas fa-signature', false);
            this.actionSignUSB = new ActionItem(17, res['ATTACHEMENT.signatureUSB'], 'background', false);
            this.actionSignCertif = new ActionItem(18, res['ATTACHEMENT.signatureCertif'], 'fas fa-file-signature', false);

            let subItemsSig: ActionItem[] = [this.actionSignDigigo, this.actionSignUSB, this.actionSignCertif];
            this.signFile = new ActionItem(19, res['ATTACHEMENT.sign'], 'fas fa-pen-nib', false, subItemsSig);
    ...
    onItemClick({itemData, viewArea, fileSystemItem}) {
        ...
        } else if (button == 'fas fa-signature') {
            this.openframeDIGIGO(fileSystemItem.dataItem);
        } else if (button == 'background') {
            this.openframeUSB(fileSystemItem.dataItem);
        } else if (button == 'fas fa-file-signature') {
            this.openframeCERTIF(fileSystemItem.dataItem);
    ...
    fileManager_onSelectionChanged(e) {
        if (e.selectedItems.length > 0) {
            this.actionItems = [this.actionViwer, this.actiongestionPDF, this.actionUpdate, this.actionDelete, this.actiondownlowd, this.actioncloned, this.actionTrs, this.actionUnTrs, this.actionVer, this.actionDeve, this.modifierFile, this.uploadFile, this.signFile];
            ...
            let indexSignPdf = array.map(x => x.id).indexOf(10);/*id ==> 10 Actions Pdf*/
            if ((e.selectedItems[0].dataItem.size === null || (e.selectedItems[0].dataItem.type === null || e.selectedItems[0].dataItem.type != 'application/pdf' || !this.isDesktopDevice)
                && indexSignPdf != undefined && indexSignPdf != null && indexSignPdf != -1) || !this.cansigDigigo) {
                array.splice(indexSignPdf, 1);
                e.component.option('contextMenu.items', array);
            }
...
// A voir
    ...
    fermerSignatureModal() {
        this.digigoPopUp = false;
        this.translateService.get('ATTACHEMENT.signErr').subscribe((res) => {
            this.toastr.error(res, '', {
                closeButton: true,
                positionClass: 'toast-top-right',
                extendedTimeOut: this.env.extendedTimeOutToastr,
    ...
    activeStepIndex() {
        if (this.cookieService.get('activeStepIndex') != null && this.cookieService.get('activeStepIndex') != undefined && this.cookieService.get('activeStepIndex') != '') {

            if (this.signatureType === 'DIGIGO') {
                if ((this.cookieService.get('activeStepIndex')) === '3') {
                    return true;
                } else {
                    return false;
                }
            } else if (this.signatureType ==='File Certificat' || this.signatureType === 'USB Token') {
                if ((this.cookieService.get('activeStepIndex')) === '2') {
                    return true;
                } else {
                    return false;
                }
    ...
    saveSignedAttachement() {
        ...
        this.digigoPopUp = false;
        if (this.signatureUUID != null && this.signatureUUID != undefined && this.signatureUUID != '') {
            ...
            this.fileservice.extractSignedAttachment(this.signatureUUID).subscribe(async (response: any) => {
                ...
                this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.save_delete', 'ATTACHEMENT.signErr').then(data => {
                    this.refresh();
    ...
    openframeDIGIGO(itemData) {
        try {
                                this.cookieService.set("signature-UUID", null, 0, "/", window.location.hostname.substring(window.location.hostname.indexOf('.')))

            this.idFileViewer = itemData.id;
            this.loadingVisible = true;
            this.fileservice.extractfileByIdJson(itemData.id, this.fileAccessToken).subscribe(async (response: any) => {
                this.FileNameToSigned = itemData.fileName;
                let base64 = this.communService.arrayBufferToBase64(new Uint8Array(response));
                let arraybuffer = this.communService.base64ToArrayBuffer(base64);
                let file = new File([arraybuffer], this.FileNameToSigned, {type: 'application/pdf'});
                this.signatureType = 'DIGIGO';
                let obj = new FormData();
                obj.append('forSignedFile', file);
                obj.append('typeSignature', 'DIGIGO');
                this.fileservice.initSignature(obj).subscribe((response: any) => {
                    this.signatureUUID = response.id;

                    this.cookieService.set('signature-UUID', this.signatureUUID, null, '/', window.location.hostname.substring(window.location.hostname.indexOf('.')));

                    this.digigoPopUp = true;
                    this.externalLinkUrl = this.sanitizer.bypassSecurityTrustResourceUrl(this.env.apiFrontPSSIGN + 'AddSignatureNotUploadFile?externaleFrame=true');
                }, error => {
                ...
                this.digigoPopUp = true;
                this.translateService.get('ATTACHEMENT.signInit', this.Ref).subscribe((res) => {
                    this.loadingVisible = false;
    ...
    openframeUSB(itemData) {
        try {
            ...
            this.fileservice.extractfileByIdJson(itemData.id, this.fileAccessToken).subscribe(async (response: any) => {
                ...
                let arraybuffer = this.communService.base64ToArrayBuffer(base64);
                let file = new File([arraybuffer], this.FileNameToSigned, {type: 'application/pdf'});
                this.signatureType = 'USB Token';
                let obj = new FormData();
                obj.append('forSignedFile', file);
                obj.append('typeSignature', 'USB_Token');
                this.fileservice.initSignature(obj).subscribe((response: any) => {
                    this.signatureUUID = response.id;
                    this.cookieService.set('signature-UUID', this.signatureUUID, null, '/', window.location.hostname.substring(window.location.hostname.indexOf('.')));
                    this.digigoPopUp = true;
                    this.externalLinkUrl = this.sanitizer.bypassSecurityTrustResourceUrl(this.env.apiFrontPSSIGN + 'AddSignatureNotUploadFileUSB?externaleFrame=true');
                }, error => {
                    this.toastr.error(error.message, '', {
                        closeButton: true,
                        positionClass: 'toast-top-right',
    ...
    openframeCERTIF(itemData) {
        try {
            ...
            this.fileservice.extractfileByIdJson(itemData.id, this.fileAccessToken).subscribe(async (response: any) => {
                ...
                let arraybuffer = this.communService.base64ToArrayBuffer(base64);
                let file = new File([arraybuffer], this.FileNameToSigned, {type: 'application/pdf'});
                this.signatureType = 'File Certificat';

                let obj = new FormData();
                obj.append('forSignedFile', file);
                obj.append('typeSignature', 'File_Certif');
                this.fileservice.initSignature(obj).subscribe((response: any) => {
                    this.signatureUUID = response.id;
                    this.cookieService.set('signature-UUID', this.signatureUUID, null, '/', window.location.hostname.substring(window.location.hostname.indexOf('.')));
                    this.digigoPopUp = true;
                    this.externalLinkUrl = this.sanitizer.bypassSecurityTrustResourceUrl(this.env.apiFrontPSSIGN + 'AddSignatureNotUploadFileCerifLocal?externaleFrame=true');
                }, error => {
                    this.toastr.error(error.message, '', {
                        closeButton: true,
                        positionClass: 'toast-top-right',
    ...
    Cloning(itemData) {
        ...
        this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.cloned', null).then(data => {
            // if (data["statut"] == true) {
            this.refresh();
            // }
        });
    ...
    transferable(itemData) {
        ...
        this.fileservice.getFileById(itemData.id).subscribe((file: any) => {
            ...
            this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.Transferable', null).then(data => {
                this.refresh();
    ...
    Untransferable(itemData) {
        ...
        this.fileservice.getFileById(itemData.id).subscribe((file: any) => {
            ...
            let paramsHttp = new HttpParamMethodPatch(this.env.apiUrlkernel + 'attachements/' + jsonFile.id, jsonFile);
            this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.UnTransferable', null).then(data => {
                this.refresh();
    ...
    locked(itemData) {
        ...
        this.fileservice.getFileById(itemData.id).subscribe((file: any) => {
            ...
            let paramsHttp = new HttpParamMethodPatch(this.env.apiUrlkernel + 'attachements/' + jsonFile.id, jsonFile);
            this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.locked', null).then(data => {
                this.refresh();
    ...
    unlocked(itemData) {
        ...
        this.fileservice.getFileById(itemData.id).subscribe((file: any) => {
            ...
            this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.Unlocked', null).then(data => {
                this.refresh();
    ...
    OfficeTemplatePopUpOpen() {
        ...
        this.httpServicesComponent.method(paramsHttp, '', 'ATTACHEMENT.OfficeTemplateSucces', null).then(data => {
            if (data['statut'] == true) {
...
// simulate a click on the link to initiate the download
    ...
    Confirmdelete() {
        ...
        this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.deleted', null).then(data => {
