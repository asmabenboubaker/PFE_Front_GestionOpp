...
export class DataGridAttachmentsComponent implements OnInit {
    ...
    constructor(private sanitizer: DomSanitizer, private deviceService: DeviceDetectorService, private env: EnvService, private http: HttpClient,
        ...
        this.isAttachmentExistForSign = this.isAttachmentExistForSign.bind(this);
        // this.lazyy();
        ...
        this.translateService.get(['ATTACHEMENT.signatureDigogo', 'ATTACHEMENT.signatureUSB', 'ATTACHEMENT.signatureCertif']).subscribe((res) => {
            let itemTitle = res;

            this.itemsListSign.push({
                id: 1,
                text: itemTitle['ATTACHEMENT.signatureDigogo'],
                icon: 'fas fa-signature',
            });
            this.itemsListSign.push({
                id: 2,
                text: itemTitle['ATTACHEMENT.signatureUSB'],
                icon: 'background',
            });
            this.itemsListSign.push({
                id: 3,
                text: itemTitle['ATTACHEMENT.signatureCertif'],
    ...
    valueChangedSecuriteLevel(data) {
        ...
        this.httpServicesComponent.method(paramsHttp, this.Ref, 'ATTACHEMENT.NiveausecuriteMiseajour', null).then(data => {
            this.refresh();
        });
    }
    ...
    viewSelectedChange(e: any) {
        if (e.selectedRowsData.length != 0) {
            this.selectedRowKeys = e.selectedRowsData[0];
        }
    }
        ...
        if(this.actionVisible) {
            if (evt.column.dataField === 'Actions' || evt.column.dataField === 'securiteLevel') {
                ...
                if (fileType === 'application/json' || fileType === 'text/plain' || fileType === 'image/jpeg' ||
                    ...
                    if (this.isactionPdf(evt)) {
                        this.viewFile(evt);
                    }
                } else {
                    if (this.isVisible(evt)) {
    ...
    isAttachmentExistForSign(data) {
        return data.row.data.fileType && data.row.data.docSize != null && this.canModiffile ;
    }
    ...
    async openOptionSign(e, data) {
        ...
        try {
            if (e.itemData.id == 1) {
             this.openframeDIGIGO(data)
            } else if (e.itemData.id == 2) {
               this.openframeUSB(data)
            } else if (e.itemData.id == 3) {
             this.openframeCERTIF(data)
            }
        } catch (error) {
            this.Ref.value = data.fileName;
            this.translateService.get("error", this.Ref).subscribe((res) => {
                this.toastr.error(res, ' ', {
                    closeButton: true,
                    positionClass: 'toast-top-right',
                    extendedTimeOut: this.env.extendedTimeOutToastr,
    ...
    onToolbarPreparinggridfile(e) {
        ...
        if (this.ContainerViewer && this.canAddfile) {
            e.toolbarOptions.items.unshift(
                {
                    location: 'after',
                    widget: 'dxButton',
        ...
        if (this.flipedDatagridButtonVisible) {
            e.toolbarOptions.items.unshift(
                {
                    location: 'after',
                    template: 'bouttonaKanBan'
                }
            );
        }
        ...
        e.toolbarOptions.items.unshift(
            ...
            // },
            {
                ...
                options: {
                    hint: 'reset',
                    icon: 'undo',
                    onClick: this.resetGridFile.bind(this),
                }
            }
        );
    }
